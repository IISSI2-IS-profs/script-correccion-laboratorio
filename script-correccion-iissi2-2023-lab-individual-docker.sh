...
cp -R ./monorepo-base/.git ./monorepo-base/.gitignore .
cd ./monorepo-base
git checkout docker
cp -f package.json Dockerfile docker-compose.yml ../
cp -f DeliverUS-Backend/.env.docker ../$carpeta_backend/
cd ..
rm $carpeta_backend/.env
docker-compose down
rm -rf ./monorepo-base
...
read -p "El script de inicialización de GIT finalizó con éxito. ¿Quiere hacer npm install y start en los dos proyectos? y/n " yn
    case $yn in
        [Yy]* ) echo "Abriendo VSCode...";code .; npm run freshStart:all:docker:bash;;
